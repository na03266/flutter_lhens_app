...
사용자가 **'위험 신고'**를 작성하고 등록 버튼을 누르면, 다음과 같은 여행을 떠나게 됩니다.
...
2.  **전송 (Transport)**:
    ...
    -   앱은 이 봉투를 인터넷을 통해 **서버(LH EnS 본부 컴퓨터)**로 발송합니다.
...
│
├── risk/                    # [Domain] 위험성 평가
│   ├── view/                # 위험성 평가 목록 및 상세, 등록 화면
│   ├── provider/            # 위험성 평가 CRUD 상태 관리
│   ├── repository/          # 위험성 평가 API
│   └── model/               # Risk 모델
│
├── chat/                    # [Domain] 채팅
├── alarm/                   # [Domain] 알림
...
### 2. 위험성 평가 (Risk) / `board-risk`
| 메서드 | 엔드포인트 | 설명 | 헤더 요구 |
| :--- | :--- | :--- | :---: |
| `GET` | `/board-risk` | 위험성 평가 목록 조회 (페이지네이션 지원) | Access |
| `GET` | `/board-risk/{id}` | 위험성 평가 상세 조회 | Access |
| `POST` | `/board-risk` | 새로운 위험성 평가 게시물 등록 | Access |
