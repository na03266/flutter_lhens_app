...
class ManualDetailScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ManualDetailScreen> createState() => _ManualDetailScreenState();
}
...
class _ManualDetailScreenState extends ConsumerState<ManualDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return ReportDetailScaffoldV2.fromModel(
      model: state,
      onUpdate:
          me is UserModel && (me.mbLevel >= 4 || state.wrName.contains(me.mbId))
          ? () {
              context.goNamed(
      ...
      me is UserModel && (me.mbLevel >= 4 || state.wrName.contains(me.mbId))
          ? () {
        try {
          ref.read(manualProvider.notifier).deletePost(wrId: widget.wrId);
      ...
      postComment: (wrId, dto) {
        ref.read(manualProvider.notifier).postComment(wrId: wrId, dto: dto);
      },
      ...
      onCommentDelete: (item) async {
        ...
        if (ok == true) {
          ...
          await ref.read(manualProvider.notifier).getDetail(wrId: widget.wrId);
        }
      },
      onCommentUpdate: (id, item) async {
        await ref.read(manualProvider.notifier).patchPost(wrId: id, dto: item);
      },
    );
  }
